...
- Controller 클래스에 DAO를 주입할 수 있도록 인스턴스 변수와 셋터를 추가한다.
  ...
  - LectureXxxController.java에 LectureDao 객체를 저장할 변수와 셋터 추가
...
- AbstractDao.java (추가)
  - ContactDao와 LectureDao의 공통 멤버를 추출하여 상위 클래스 정의
  - ContactDao와 LectureDao는 AbstractDao를 상속 받는다.
- ContactXxxController와 LectureXxxController 변경
  - ContactDao와 LectureDao 변경 사항에 맞춰 호출 코드 변경
...
-  bitcamp/java89/ems/server/vo 패키지 생성
  -  contact.java, Book.java를 이 패키지로 옮긴다.
-  bitcamp/java89/ems/server/controller 패키지 생성
  -  ContactController.java, BookController.java를 이 패키지로 옮긴다.
...
####구현할 기능:
- LinkedList 클래스를 만든다.
- LinkedList를 BookController에 적용한다.
...
- EduApp에 있던 학생관리 명령어 처리를
   학생관리 전문가인 BookController로 이전한다.
...
- 메서드나 변수에 대해 접근을 제어하는 방법과 이유를 이해한다.
  - BookController의 doBook() 메서드들을 private 으로 접근을 제한한다.
...
# v0.5
####구현할 기능:
- BookController 클래스를 확장해서 사용할 수 있도록,
...
- EduApp에 있는 Book에 대해 명령을 다루는 데 관련된 메서드를
   따로 다른 클래스로 분류한다.
...
# v0.1
####구현할 기능:
- 학생 정보를 다룰 때 사용할 Book 데이터 타입을 정의한다.
- 학생 정보를 입력 받아 출력해본다.
