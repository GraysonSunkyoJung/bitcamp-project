  ...
  public void run() {
    ...
    try {
      ...
      while (true) {
        ...
        String[] command = in.nextLine().split("\\?");; //입력받은걸 그대로 출력
        ...
        if (requestHandler == null) {
          ...
          continue; // 다음 줄로 가지 않고 반복문 조건 검사로 건너 뛴다. (while (true)38번 줄로감)
        } 
          ...
          // 2) 찾은 메서드를 호출한다. 호출할 떄 파라미터 값 2개를 넘긴다.
            m.invoke(requestHandler, paramMap, out);
            
          } catch (Exception e) {
            out.println("작업중 오류가 발생했습니다.");
            e.printStackTrace();
          }
      } //while
