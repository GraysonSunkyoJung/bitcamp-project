/* 역할 : 빈 컨데이너(Bean Container) == IoC 컨테이너
...
public class ApplicationContext {
    ...
    for (Object obj : objects) {
      ...
      for (Method m : methods) {
        ...
        if (dependency != null) { // 찾았다면,
          try {
            m.invoke(obj, dependency); // 의존객체를 주입한다.
          } catch (Exception e) {}
        }
      }
    }
  ...
  private ArrayList<Class<?>> getClassList(String[] packages) {
    ...
    for (String packageName : packages) {
      ...
      try {
        // package 경로를 file 경로로 바꾸어서 클래스 리스트를 뽑아내는 과정
  ...
  private void prepareObjects(ArrayList<Class<?>> classList) {
    for (Class<?> clazz : classList) {
      try {
        ...
        // 애노테이션의 값을 저장할 때는 변수처럼, 값을 꺼낼 때는 메서드 처럼 사용한다.
 ...
 // 클래스 파일을 찾을 디렉토리 경로를 정의한다.
  ...
  private boolean isComponent(Class<?> c) {
    // @Component 애노테이션이 있다면 리턴값은 null 이 아니다.
